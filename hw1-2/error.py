...
with tf.Session() as sess:
    ...
    for i in range(1, nb_epoch+1):
        for b in tqdm(range(n_batch)):
            _, loss, train_acc = sess.run([train_op, loss_op, accuracy],
    ...
    # test_acc = sess.run(accuracy, feed_dict={X: x_test, y_true_test: y_test})
    # print(test_acc)
...
# 		plt.plot(x_plot, model[i].predict(x_plot))
