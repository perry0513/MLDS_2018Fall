...
class DCGAN():
	...
	def discriminator(self, inputs, condition, reuse=False, is_training=True):
		with tf.variable_scope('discriminator', reuse=reuse):
			...
			conv1 = tf.nn.leaky_relu((tf.layers.conv2d(inputs=inputs, filters=64, kernel_size=(5,5), strides=(2,2), 
...
# 			conv1 = tf.layers.dropout(conv1, rate=self.dropout_rate, training=is_training)
			conv2 = tf.nn.leaky_relu((tf.layers.conv2d(inputs=conv1, filters=128, kernel_size=(5,5), strides=(2,2), 
													   kernel_initializer=tf.truncated_normal_initializer(stddev=0.02))), alpha=self.alpha)
# 			conv2 = tf.layers.dropout(conv2, rate=self.dropout_rate, training=is_training)
			conv3 = tf.nn.leaky_relu((tf.layers.conv2d(inputs=conv2, filters=256, kernel_size=(5,5), strides=(2,2), 
													   kernel_initializer=tf.truncated_normal_initializer(stddev=0.02))), alpha=self.alpha)
#############################################################################
			tiled_embeddings = tf.tiles(condition, [1, 8, 8, 1])
			concat = tf.concat([conv3, tiled_embeddings], -1)
#############################################################################
# 			conv3 = tf.layers.dropout(conv3, rate=self.dropout_rate, training=is_training)
			conv4 = tf.nn.leaky_relu((tf.layers.conv2d(inputs=concat, filters=512, kernel_size=(5,5), strides=(2,2), 
													   kernel_initializer=tf.truncated_normal_initializer(stddev=0.02))), alpha=self.alpha)
...
#############################################################################		
		self.label = tf.placeholder(shape=(None, ), dtype=tf.float32, name='train_label')
		self.infer_label = tf.placeholder(shape=(None, ), dtype=tf.float32, name='infer_label')
#############################################################################
...
# 		false_label = tf.random_uniform(tf.shape(self.d_fake), 0, 0.3)
		...
		self.g_infer = self.generator(self.g_inputs, self.infer_label, tf.AUTO_REUSE, is_training=False)
